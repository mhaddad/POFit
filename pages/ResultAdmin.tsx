          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
            ...
            <div className="bg-white rounded-3xl border border-slate-100 shadow-sm overflow-hidden flex flex-col h-full">
              ...
              <div className="p-6 flex-grow space-y-4">
                <div>
                  ...
                  <p className="text-sm font-bold text-slate-700">{data.ipaStatus}</p>
                </div>
                <div>
                  <h4 className="text-[10px] font-black text-slate-400 uppercase tracking-widest mb-1">Camada Dupla (Índice + Âncora B10)</h4>
                  <p className="text-xs text-slate-500 leading-relaxed">
                    {data.ipa < 3.0 && data.blockScores['B10'] > 3.5 ? "Aderente via Design (Vontade alta, mas organização oscila)" :
                      data.ipa < 3.0 && data.blockScores['B10'] <= 3.5 ? "Desalinhamento Sistêmico (Inseguro em ambientes autônomos)" :
                        "Equilíbrio Estrutural (Autonomia Saudável)"}
                  </p>
                </div>
            ...
            <div className="bg-white rounded-3xl border border-slate-100 shadow-sm overflow-hidden flex flex-col h-full">
              ...
              <div className="p-6 flex-grow space-y-4">
                <div>
                  <h4 className="text-[10px] font-black text-slate-400 uppercase tracking-widest mb-1">Status</h4>
                  <p className="text-sm font-bold text-slate-700">{data.irccStatus}</p>
                </div>
                <div>
                  <h4 className="text-[10px] font-black text-slate-400 uppercase tracking-widest mb-1">Camada Dupla (Índice + Âncora B7)</h4>
                  <p className="text-xs text-slate-500 leading-relaxed">
                    {data.ircc < 3.0 && data.blockScores['B7'] > 3.5 ? "Aderente via Design (Exige clareza de contexto)" :
                      data.ircc < 3.0 && data.blockScores['B7'] <= 3.5 ? "Desalinhamento Sistêmico (Sensível a mudanças caóticas)" :
                        "Equilíbrio Estrutural (Resiliência Saudável)"}
                  </p>
                </div>
                <div className="pt-4 mt-auto border-t border-slate-50">
                  <h4 className="text-[10px] font-black text-slate-400 uppercase tracking-widest mb-2">Ações Recomendadas</h4>
            ...
            <div className="bg-white rounded-3xl border border-slate-100 shadow-sm overflow-hidden flex flex-col h-full">
              ...
              <div className="p-6 flex-grow space-y-4">
                <div>
                  <h4 className="text-[10px] font-black text-slate-400 uppercase tracking-widest mb-1">Status</h4>
                  <p className="text-sm font-bold text-slate-700">{data.iiseStatus}</p>
                </div>
                <div>
                  <h4 className="text-[10px] font-black text-slate-400 uppercase tracking-widest mb-1">Camada Dupla (Índice + Âncora B5)</h4>
                  <p className="text-xs text-slate-500 leading-relaxed">
                    {data.iise < 3.0 && data.blockScores['B5'] > 3.5 ? "Aderente via Design (Comunicação Assíncrona preferida)" :
                      data.iise < 3.0 && data.blockScores['B5'] <= 3.5 ? "Desalinhamento Sistêmico (Risco de gargalos de comunicação)" :
                        "Equilíbrio Estrutural (Colaboração Saudável)"}
                  </p>
                </div>
                <div className="pt-4 mt-auto border-t border-slate-50">
                  <h4 className="text-[10px] font-black text-slate-400 uppercase tracking-widest mb-2">Ações Recomendadas</h4>
              ...
              <div className="mt-6 p-5 bg-slate-50 rounded-2xl border border-dashed border-slate-200 shadow-sm">
                <div className="flex flex-col gap-2">
                  ...
                  <div className="mt-2 text-xs text-slate-600 leading-relaxed italic border-l-2 border-primary/20 pl-3">
                    <p>{data.subQuadrant && POWER_PLACES[data.subQuadrant] ? POWER_PLACES[data.subQuadrant] : "Lugar de potência analisado."}</p>

                    {data.riskPersonas && data.riskPersonas.length > 0 && (
                      <div className="mt-3 p-2 bg-rose-50 rounded-lg border border-rose-100">
                        <p className="font-bold text-rose-800 text-[10px] uppercase">Personas de Risco Detectadas:</p>
                        <p className="text-rose-700 font-medium">{data.riskPersonas.join(', ')}</p>
                      </div>
                    )}

                    {data.onboardingModel && (
                      <div className="mt-3 p-2 bg-indigo-50 rounded-lg border border-indigo-100">
                        <p className="font-bold text-indigo-800 text-[10px] uppercase">Modelo de Onboarding Recomendado:</p>
                        <p className="text-indigo-700 font-medium">Modelo {data.onboardingModel}</p>
                      </div>
                    )}
